# --- Pub acl rules ---
resource "aws_network_acl_rule" "pub_sub_ssh_rule" {
    network_acl_id = aws_network_acl.pub_sub_acl.id
    rule_number    = 200
    protocol       = "tcp"
    rule_action    = "allow"
    egress         = false
    cidr_block     = "5.161.73.127/32"
    from_port      = 22
    to_port        = 22
}
...
resource "aws_network_acl_rule" "k8s_sub_internet_rule" {
...
resource "aws_network_acl_rule" "k8s_sub_ssh_rule" {
    network_acl_id = aws_network_acl.k8s_sub_acl.id
    rule_number    = 200
    protocol       = "tcp"
    rule_action    = "allow"
...
# --- dp acl rules ---
resource "aws_network_acl_rule" "dp_sub_internet_rule" {
...
resource "aws_network_acl_rule" "sig_sub_internet_rule" {
